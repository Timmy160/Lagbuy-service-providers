...
function PaymentSuccess() {
  ...
  // Get user's name from auth or fallback
  const userName = user?.displayName?.split(" ")[0] || user?.email?.split("@")[0] || "there";

  // Get course info from localStorage (set during payment)
  const courseTitle = localStorage.getItem("courseTitle") || "The Bu$iness of Content Creation";
  const courseTutor = localStorage.getItem("courseTutor") || "Joses Frank-Ndubisi";
          ...
          <div className="p-8 md:p-10 lg:p-12 text-center space-y-8">
            ...
            <div className="space-y-3">
              ...
              <p className="text-lg text-[#1A362B]/50 leading-relaxed">
                Welcome to <span className="font-bold text-[#1A362B]">{courseTitle}</span> with <span className="font-semibold text-[#1A362B]">{courseTutor}</span>.
              </p>
            </div>
            ...
            <div className="bg-[#F5F5F0]/50 backdrop-blur-sm rounded-2xl border border-[#1A362B]/5 p-6 text-left space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#1A362B]">Message on WhatsApp</p>
                  <p className="text-xs text-[#1A362B]/40 mt-0.5">Click the button below to send a direct message</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#1A362B]">Drop Your Payment Receipt</p>
                  <p className="text-xs text-[#1A362B]/40 mt-0.5">Send a screenshot of your payment confirmation in the DM</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#1A362B]/40 mt-0.5">We'll verify your payment and send your course access within 24 hours</p>
                </div>
              </div>
            </div>
            ...
            {/* WhatsApp DM CTA */}
            <a
              href="https://wa.me/qr/REYDJILWSU7XP1"
              target="_blank"
              rel="noreferrer"
            ...
            <p className="text-sm text-[#1A362B]/30 font-medium">
              Keep your payment receipt handy — you'll need it in the DM.
            </p>

            <button
              onClick={() => navigate("/course-details")}
              className="text-sm text-[#1A362B]/40 hover:text-[#1A362B]/60 font-medium transition-colors"
            >
              ← Back to Course
            </button>
          </div>
